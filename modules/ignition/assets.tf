...
data "template_file" "k8s_node_bootstrap" {
  ...
  vars {
    ...
    kubeconfig_fetch_cmd     = "${var.kubeconfig_fetch_cmd != "" ? "ExecStartPre=${var.kubeconfig_fetch_cmd}" : ""}"
    tectonic_torcx_image_url = "${var.quay_hostname == "" ? "quay.io" : var.quay_hostname}${replace(var.container_images["tectonic_torcx"],var.image_re,"$1")}"
    tectonic_torcx_image_tag = "${var.quay_hostname == "" ? "quay.io" : var.quay_hostname}${replace(var.container_images["tectonic_torcx"],var.image_re,"$2")}"
...
data "template_file" "s3_puller" {
  ...
  vars {
    awscli_image = "${var.quay_hostname == "" ? "quay.io" : var.quay_hostname}${var.container_images["awscli"]}"
  }
}
...
data "template_file" "installer_kubelet_env" {
  ...
  vars {
    kubelet_image_url = "${var.quay_hostname == "" ? "quay.io" : var.quay_hostname}${replace(var.container_images["hyperkube"],var.image_re,"$1")}"
    kubelet_image_tag = "${var.quay_hostname == "" ? "quay.io" : var.quay_hostname}${replace(var.container_images["hyperkube"],var.image_re,"$2")}"
  }
}
