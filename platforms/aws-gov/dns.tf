data "null_data_source" "zones" {
  inputs = {
    private = "${var.tectonic_aws_external_private_zone == "" ? join("", aws_powerdns_zone.tectonic_int.*.zone_id) : var.tectonic_aws_external_private_zone}"
    public  = "${join("", data.aws_powerdns_zone.tectonic_ext.*.zone_id)}"
  }
}

data "aws_powerdns_zone" "tectonic_ext" {
  count = "${var.tectonic_aws_public_endpoints}"
  name  = "${var.tectonic_base_domain}"
}

resource "aws_powerdns_zone" "tectonic_int" {
