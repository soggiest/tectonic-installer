...
module "ignition_powerdns" {
  ...
  kubeconfig_fetch_cmd = "/opt/s3-puller.sh ${aws_s3_bucket_object.kubeconfig.bucket}/${aws_s3_bucket_    object.kubeconfig.key} /etc/kubernetes/kubeconfig"
}
...
module "powerdns" {
  ...
  sg_ids                       = "${concat(var.tectonic_aws_worker_extra_sg_ids, list(module.vpc.worke    r_sg_id))}"
  ssh_key                      = "${var.tectonic_aws_ssh_key}"
  subnet_ids                   = "${module.vpc.worker_subnet_ids}"
  vpc_id                       = "${module.vpc.vpc_id}"
  worker_iam_role              = "${var.tectonic_aws_worker_iam_role_name}"
  load_balancers               = ["${var.tectonic_aws_powerdns_load_balancers}"]

  ign_docker_dropin_id              = "${module.ignition_workers.docker_dropin_id}"
  ign_locksmithd_service_id         = "${module.ignition_masters.locksmithd_service_id}"
  ign_max_user_watches_id           = "${module.ignition_workers.max_user_watches_id}"
  ign_s3_puller_id                  = "${module.ignition_workers.s3_puller_id}"
  ign_powerdns_service_id           = "${module.ignition_powerdns.powerdns_service_id}"
}

provider "powerdns" {
  version = "0.1.0"
  api_key = ""
  server_url = "${aws_instance.powerdns_node.1.private_ip[0]}"
}
...
module "ignition_masters" {
  source = "../../modules/aws-gov/ignition"
